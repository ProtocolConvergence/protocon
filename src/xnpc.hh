...
public:
  Table<std::string> keys;
  Table<NatPredicateMap> vals;
  Map<std::string,uint> map;

  void add(const std::string& key, const NatPredicateMap& val) {
    keys.push(key);
    vals.push(val);
    map[key] = keys.sz()-1;
  }

  NatPredicateMap* lookup(const std::string& key) {
...
public:
  Table<std::string> keys;
  Table<P::Fmla> pfmlas;
  Table<std::string> expressions;
  Map<std::string,uint> map;

  void add(const std::string& key, const P::Fmla& pf, const std::string& expression) {
    keys.push(key);
    pfmlas.push(pf);
  ...
  P::Fmla* lookup(const std::string& key) {
    uint* idx = map.lookup(key);
    if (!idx)  return 0;
    return &pfmlas[*idx];
  }
...
inline std::string name_of(const Vbl& vbl) {
  return vbl.symm->spec->name + '[' + std::to_string(vbl.symm_idx) + ']';
}
...
public:
  ...
  std::string vbl_name(uint i) const {
    std::string s = rvbl_symms[i]->spec->name;
    s += '[';
    s += vbl_indices[i].expression();
    s += ']';
    return s;
  }
...
inline std::string name_of(const Pc& pc) {
  return (
      pc.symm->spec->name + '[' +
      std::to_string(pc.symm->mapped_indices.eval(pc.symm_idx)) + ']');
}
