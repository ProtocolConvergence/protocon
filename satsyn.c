    ...
    { BLoop( pcidx, sys->pcs.sz )
        ...
        { BLoopT( XnSz, rule_step_idx, n_rule_steps )
            ...
            { BLoop( i, fmla->clauses.sz )
                ...
                CnfDisj clause = fmla->clauses.s[i];
                { BLoop( j, 3 )
                    bool found = false;

                    { BLoop( k, 3 )
                        if (g.p.s[x_pidcs[j]] == clause.terms.s[j].vbl)
                        {
                            if (g.p.s[y_pidcs[j]] == clause.terms.s[j].val)
                                satisfied = true;
                            found = true;
                        }
                    } BLose()
